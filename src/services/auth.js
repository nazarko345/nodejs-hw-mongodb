...
import { sendEmail } from '../utilts/sendMail.js';
import handlebars from 'handlebars';
import path from 'node:path';
import fs from 'node:fs/promises';
import { getFullNameFromGoogleTokenPayload, validateCode } from '../utilts/googleOAuth2.js';
...
export const requestResetToken = async (email) => {
  const user = await UserCollection.findOne({ email }); // ✅ шукаємо у UserCollection і додаємо await
  if (!user) {
